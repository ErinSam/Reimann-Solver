...
def plotter(U, count, time)
    ...
    plt.subplot(311)
    plt.plo(X, W[1:-1,0], 'bo-', linewidth=0.25, markersize=0.25)
    plt.title("Density vs Position")
    plt.ylabel("Density (kg/m^3)")
    plt.xlabel("Position (m)")

    plt.subplot(312)
    plt.plo(X, W[1:-1,1], 'bo-', linewidth=0.25, markersize=0.25)
    plt.title("Velocity vs Position")
    plt.ylabel("Velocity (m/s^2)")
    plt.xlabel("Position (m)")
    
    plt.subplot(313)
    plt.plo(X, W[1:-1,2], 'bo-', linewidth=0.25, markersize=0.25)
