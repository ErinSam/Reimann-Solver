"""
    ...
    Version 3: 
        Removing the mistake in the iteration procedure. Previous version started 
        iterating for conditions left (and right) of the contact discontinuity at 
        x/t = 0, rather than at x/t = u*, which is the wave speed for the contact 
        discontinuity.
"""
...
def exact_reimann_solver():
    ...
    plt.subplot(311)
    plt.plot(X, W[0,], 'bo-', linewidth=0.5, markersize=0.75)
    plt.title("Exact solution for Pressure")
    plt.ylabel("Pressure")
    plt.xlabel("Position")
    
    plt.subplot(312)
    plt.plot(X, W[1,], 'bo-', linewidth=0.5, markersize=0.75)
    plt.title("Exact solution for Velocity")
    plt.ylabel("Velocity")
    plt.xlabel("Position")

    plt.subplot(313)
    plt.plot(X, W[2,], 'bo-', linewidth=0.5, markersize=0.75)
